...
笔者当前是一位处于天然产物全合成课题组的在读博士生。在本科阶段，笔者的导师时常与我畅想“未来的有机化学”，这一宏大愿景下的一项命题即为“计算辅助的有机化合物的结构解析”，在那时我们便设想搭建一个以实用性为导向的面向有机合成课题组的计算化学辅助的有机化合物结构解析平台。然而，尽管积累了很多素材、知识和经验，受各种不可抗力的影响下，这一计划一拖再拖，一直到笔者本科毕业，最终也未能去完成这一实践。巧合的是，在研究生阶段，笔者所在课题组遇到了目标分子原始结构鉴定有误的难题，笔者提出可以通过核磁计算的方式重新解析天然产物结构，最终揭示出若干个不同寻常的结构突变并得到了合成化学的印证（*J. Am. Chem. Soc.* **2025**, *147*, 33136.）。通过此次偶然的实践，笔者所处的一个传统的天然产物全合成课题组与核磁计算的工具建立起了牢靠的信任关系。事实上，在日常的合成研究中，每一步反应所得的高级中间体，其结构之复杂与精巧并不亚于天然产物本身，而计算亦从不区分人工与天然，因此这一实用的工具有望用于辅助课题组成员对于合成中间体的结构鉴定。笔者为此重新拾起本科时期的设想，充分考虑一个忙碌的有机合成课题组的日常需求，花费了数月的构思、学习与尝试，选取了若干实用的核磁计算流程，通过将自编的批处理脚本和现有程序相结合的方式，为组内初步搭建了一套自动化核磁计算平台，实现了从输入记录化合物结构的.xyz文件到输出易于处理的数据文件的一键化计算，现将这一初步的成果和相应的使用经验与大家分享，欢迎各位批评指正。

### 1. 简介

#### 1.1 **有机合成化学家为什么需要核磁计算？**

在各种有机化学或仪器分析的教材中，我们学习了很多实用的现代表征技术用于有机化合物的结构鉴定，如红外、拉曼、紫外、核磁、质谱、X-射线单晶衍射等。在实践中，面临一个结构待定的新有机化合物，核磁往往成为有机化学家鉴定结构的首选。抛开技术发展和测试成本问题，笔者认为核磁成为首选的一个重要原因是其可预见性相较其他表征手段更强，即在有机化学家的知识体系下，一个给定结构的有机化合物对应的特征核磁信号可以在人脑中得到直观的预测，预测值与实验值频频自洽便加强了有机化学家对结构研判无误的信心。因此，有机化学家对核磁谱图的解析能力的强弱与其预测水平的高低密切相关，而核磁计算本身就是在对核磁信号进行预测。值得注意的是，对于长程耦合的强弱、同类质子的化学位移的相对大小等精细的结构信息，即便是经验丰富的有机化学家也未必能给出可靠的预测，而核磁计算总能给出可参考的答案。总之，在核磁计算的辅助下，有机化学家可以从更多的角度去发掘预测值与实验值的自洽，进一步提高对有机化合物结构的解析能力。

#### 1.2 有机合成化学家需要什么精度的核磁计算？

溶液体系中分子的行为错综复杂，通过量子化学计算的方式研究有机分子在溶液体系中的磁学性质固然基于一些模型、假设和近似处理。有机合成化学家追求的”高精度“核磁计算并不是要求原理上的严格和精确，而是寻求在某种处理方式下获得的预测值与实验值有一定的可比性。更有甚者，这一可比性也可以不必关心，仅关注核磁计算与实验结论的关联即可。换言之，提高核磁计算在数值上的准确度是计算化学家需要克服的问题，有机合成化学家的关注点应在于如何应用核磁计算为有机物结构鉴定作出可靠判断。在这一思路的引导下，笔者于此构建的若干计算模块，其出发点是通过一套标准的、成熟的、普适的核磁计算和数据处理流程直接获取与有机物结构鉴定相关的结论，并不对数值精度和原理问题进行额外的拓展。
...
由于Chem3D不能直接生成.xyz文件，笔者一般通过其生成的.mol文件利用Multiwfn转换为.xyz文件，进入安装Multiwfn程序的文件夹，创建文本文件xyz_selection.txt，写入内容：
...
在个人电脑上安装DP4plus-App用于处理数据，安装见[https://github.com/Sarotti-Lab/DP4plus-App](https://github.com/Sarotti-Lab/DP4plus-App)。笔者发现新版本的DP4plus-App识别文件名疑似存在BUG，此处建议安装0.2.8版本。
...
此处，Isomer 1对应结构（**8a**），Isomer 2对应结构（**8b**）。P<sub>rel</sub>与DP4概率类似，可见STS判断合成结构中胺氧化物的氮手性中心的立体化学应与（**8a**）一致，这与文献报道的NOE相关亦相符。注意到对于正确匹配的结构，STS计算所得的碳化学位移预测值与实验值的平均偏差仅1 ppm左右，具有相当高的精确度，可作为检验结构正误的金标准。
